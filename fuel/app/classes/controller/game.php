...
class Controller_Game extends Controller_Template {
    ...
    public function action_view($id = null) {
        ...
        $this->template->title = $this->data['game']->team_season->teams->team_name.' '.$this->data['game']->team_season->seasons->start.'-'.$this->data['game']->team_season->seasons->end." " . ($this->data['game']->hrn == 1 ? 'Vs' : ($this->data['game']->hrn == 2 ? '@' : 'Vs')) . " " . $this->data['game']->opponents->opponent_name . " - " .date_format(date_create($this->data['game']->game_date), "m/d/Y");
