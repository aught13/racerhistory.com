...
<?php if (!empty($record['cw']) | !empty($record['cl'])): ?>
    <table class="table">
        ...
        <tr>
            ...
            <td><?=
                !empty($team_season->league_finish) ? $team_season->league_finish : '';
                ?></td>
...
<!-- SEASON NOTES -->
<?=
!empty($team_season->team_season_notes) ? 
'<div class="row">' . $team_season->team_season_notes . '</div>' : '';
?>
<hr>
<!-- RECAP AND PREVIEW -->
<div class="row">    
    <?php if (!empty($team_season->team_season_preview)): ?>
        ...
        <div id="preview" class="col-sm-12 collapse <?=
        empty($team_season->team_season_recap) ? 'in' : ''
        ?>">
...
<div class="btn-group">
    <?=
    Html::anchor(
        'admin/team/season/edit/' . $team_season->id, 'Edit Details', ['class' => 'btn btn-warning']
    );
    ?>
    <?=
    Html::anchor(
        'admin/team/season', 'Back', ['class' => 'btn btn-default']
    );
    ?>
</div>
<!-- SEASON GAMES -->
            ...
            <tbody>
                <?php foreach ($team_season->games as $item): ?>
                    <tr>
                        ...
                        <td><?= $item->mur_rk; ?> Racers <?=
                            $item->hrn == '1' ? 'Vs' : ($item->hrn == '2' ? '@' : 'Vs');
                            ?></td>
                        <td><?= $item->opp_rk; ?> <?= $item->opponents->opponent_name; ?></td>
                        <td><?=
                            (($item->game_types->conf == '1' && $item->game_types->post == '1') ? 't' : ($item->game_types->post
                                == '1' ? '^' : ($item->game_types->conf == '1' ? '*' : '')));
                            ?></td>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-condensed">
            ...
            <tbody>
                <?php
                foreach (
                    $team_season->team_season_roster as $item
                ):
                    ?>
                    <tr>
...
<div class="row">
    <?php if ($team_season->team_season_roster || $team_season->stat_basket_season_team):
        ?>
        <div class="col-sm-<?=
        $team_season->stat_basket_season_team && (isset($stats['FGA']) || isset($stats['FTA']) || isset($stats['TPA'])) ? 12
                : 9;
        ?>"> 
            <table class="table table-condensed table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        $team_season->team_season_roster ? 1 : (isset($stats['MIN']) ? 4 : 3);
                        ?>"><?= $team_season->team_season_roster ? '#' : '';
                        ?></th>
                        <?=
                        ...
                        (isset($stats['MIN']) && $team_season->team_season_roster) ? '<th rowspan="2" colspan="1">Min</th>'
                                : '';
                        ?>
                        <th class="text-center" rowspan="
                        <?=
                        isset($stats['FGA']) ? 1 : 2;
                        ?>" colspan="<?= isset($stats['FGA']) ? 2 : 1; ?>">FG</th>
                            <?=
                            isset($stats['TPM']) ? '<th class="text-center" rowspan="1" colspan="' . (isset($stats['TPA'])
                                        ? 2 : 1) . '">3P</th>' : '';
                            ?>
                        <th class="text-center" rowspan="
                        <?=
                        ...
                        ?>" colspan="<?= isset($stats['FTA']) ? 2 : 1; ?>">FT</th>
                            <?=
                            isset($stats['ORB']) ? '<th class="text-center" rowspan="1" colspan="' . (isset($stats['DRB'])
                                        ? 3 : 2) . '">Rebounds</th>' : (isset($stats['RB']) ? '<th rowspan="2" colspan="1">RB</th>'
                                        : '');
                            ?>
                            <?=
                            isset($stats['PF']) ? '<th class="text-center" rowspan="' . (isset($stats['FD']) ? 1 : 2) . '" colspan="' . (isset($stats['FD'])
                                        ? 2 : 1) . '">' . (isset($stats['FD']) ? 'Fouls' : 'PF') . '</th>' : '';
                            ?>
                            <?=
                            ...
                            ?>
                            <?=
                            isset($stats['BS']) ? '<th class="text-center" rowspan="' . (isset($stats['BD']) ? 1 : 2) . '" colspan="' . (isset($stats['BD'])
                                        ? 2 : 1) . '">' . (isset($stats['BD']) ? 'Blocks' : 'BS') . '</th>' : '';
                            ?>
                        ...
                        <?php foreach ($stats['person'] as $value): ?>
                            <?php if ($value->stat_basket_season_person): ?>
                                <tr>                    
                                    ...
                                    <td><?=
                                        Html::anchor(
                                            'admin/stat/basket/season/person/edit/' . $value->stat_basket_season_person->id,
                                            'edit', ['class' => '']
                                        );
                                        ?>|<?=
                                        Html::anchor(
                                            'admin/stat/basket/season/person/delete/' . $value->stat_basket_season_person->id,
                                            'Delete',
                                            [
                                                'onclick' => "return confirm('Are you sure?')"]
                                        );
                ...
                <?php if ($team_season->stat_basket_season_team || $team_season->stat_basket_season_opponent): ?><!-- totals -->
                    <tfoot>
                        <?php if ($team_season->stat_basket_season_team): ?>
                            <?php
                            foreach (
                                $team_season->stat_basket_season_team as $value
                            ):
                                ?>
                                <tr>
                                    <th colspan="<?= isset($stats['GS']) ? 3 : 2; ?>">TEAM TOTAL</th>
                                    <?=
                                    isset($stats['GP']) ? '<th ' . ($team_season->stat_basket_season_opponent ? ' rowspan="2" '
                                                : '') . 'class="text-center">' . $value->GP . '</th>' : '';
                                    ?>
                                    <?=
                                    isset($stats['MIN']) ? '<th ' . ($team_season->stat_basket_season_opponent ? ' rowspan="2" '
                                                : '') . 'class="text-center">' . $value->MIN . '</th>' : '';
                                    ?>
                                    <?=
                                    ...
                                    ?>
                                    <?=
                                    isset($stats['PF']) ? '<th class="text-center"' . (isset($stats['FD']) ? 'colspan="2"'
                                                : '') . '>' . $value->PF . '</th>' : '';
                                    ?>
                                    <?=
                                    ...
                                    ?>
                                    <?=
                                    isset($stats['BS']) ? '<th class="text-center"' . (isset($stats['BD']) ? 'colspan="2"'
                                                : '') . '>' . $value->BS . '</th>' : '';
                                    ?>
                                    <?=
                        ...
                        <?php if ($team_season->stat_basket_season_opponent): ?>    
                            <?php
                            foreach (
                                $team_season->stat_basket_season_opponent as $value
                            ):
                                ?>
                                <tr>
