...
<?php if ($person->team_season_roster): ?>
                    ...
                    <tr>
                        <td><?=
                            Html::anchor(
                                ...
                                ($item->team_season->semester == 1 ? $item->team_season->seasons->start
                                        : ($item->team_season->semester == 2 ? $item->team_season->seasons->start . '-' . date_format(
                                            DateTime::createFromFormat(
                                                'Y',
                                                $item->team_season->seasons->end
                                            ), 'y'
                                        ) : $item->team_season->seasons->end)) . ' - ' . $item->team_season->teams->team_name
                            );
                            ?></td>
    ...
    <div class="row">
        <?php if ($person->team_season_roster && isset($stats['PTS'])): ?>    
            ...
            <div class="col-sm-<?=
            $person->team_season_roster && (isset($stats['FGA']) || isset($stats['FTA'])
            || isset($stats['TPA'])) ? 12 : 9;
            ?> table-responsive"> 
                    ...
                    <thead>
                        <tr>
                            ...
                            ?>" colspan="<?= isset($stats['FGA']) ? 2 : 1; ?>">FG</th>
                                <?=
                                isset($stats['TPM']) ? '<th class="text-center border" rowspan="1" colspan="' . (isset($stats['TPA'])
                                            ? 2 : 1) . '">3P</th>' : '';
                                ?>
                            <th class="text-center border" rowspan="<?=
                            isset($stats['FTA']) ? 1 : 2;
                            ?>" colspan="<?= isset($stats['FTA']) ? 2 : 1; ?>">FT</th>
                                ...
                                ?>
                                <?=
                                isset($stats['PF']) ? '<th name="hide" class="text-center border d-none d-md-table-cell" rowspan="' . (isset($stats['FD'])
                                            ? 1 : 2) . '" colspan="' . (isset($stats['FD'])
                                            ? 2 : 1) . '">' . (isset($stats['FD'])
                                            ? 'Fouls' : 'PF') . '</th><th name="show" class="text-center border d-md-none"rowspan="2" colspan="1">PF</th>'
                                        : '';
                                ?>
                                <?=
                                ...
                                ?>
                                <?=
                                isset($stats['BS']) ? '<th name="hide" class="text-center border d-none d-md-table-cell" rowspan="' . (isset($stats['BD'])
                                            ? 1 : 2) . '" colspan="' . (isset($stats['BD'])
                                            ? 2 : 1) . '">' . (isset($stats['BD'])
                                            ? 'Blocks' : 'BS') . '</th><th name="show" class="text-center border d-md-none"rowspan="2" colspan="1">BS</th>'
                                        : '';
                                ?>
                            ...
                            <?php if ($value->stat_basket_season_person): ?>
                                <tr>                    
                                    ...
                                    <td class="border d-xl-none">
                                        <?=
                                        ($value->team_season->semester == 1 ? $value->team_season->seasons->start
                                                : ($value->team_season->semester
                                            == 2 ? date_format(
                                                    DateTime::createFromFormat(
                                                        'Y',
                                                ...
                                                ) : $value->team_season->seasons->end)) . ' - ' . $value->team_season->teams->abbr;
                                        ?>
                                    </td>
                                    <?=
